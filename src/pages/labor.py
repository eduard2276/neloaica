...
class LaborPage(QWidget):
    ...
    def display_labor(self, labor_list: list[dict]):
        ...
        for row, labor in enumerate(labor_list):
            ...
            edit_btn.clicked.connect(lambda checked, lab=labor: self.edit_labor_by_id(lab["id"]))
            actions_layout.addWidget(edit_btn)

            delete_btn = QPushButton("🗑️")
            delete_btn.setStyleSheet(theme.button_icon("delete"))
            delete_btn.clicked.connect(
                lambda checked, lab=labor: self.delete_labor_by_id(lab["id"])
            )
    ...
    def filter_labor(self, text: str):
        ...
        filtered = [
            ...
            if text in str(labor["id"]).lower() or text in labor["service_name"].lower()
        ]
    ...
    def edit_labor(self):
        ...
        # Find the labor data
        labor_data = next((lab for lab in self.all_labor if lab["id"] == labor_id), None)
        if not labor_data:
            return
    ...
    def delete_labor(self):
        ...
        msg_box.setText(
            f"Are you sure you want to delete the service '{labor_data['service_name']}'?"
        )
