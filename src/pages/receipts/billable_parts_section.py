...
from src.database.models.parts import add_part as add_part_to_db
from src.database.models.parts import get_all_parts, get_part_by_name
...
class BillablePartsSectionWidget(QWidget):
    ...
    def add_part(
        ...
        item_widget.setStyleSheet(
            "#billablePartItem, #billablePartItem * { background-color: white; }"
            " #billablePartItem { border-bottom: 1px solid #bdc3c7; }"
        )
        ...
        if units > 0:
            units_input.setText(str(int(units)) if units == int(units) else str(units))
        units_input.setProperty("part_id", part_id)
        units_input.textChanged.connect(
            lambda text, p_id=part_id, inp=units_input: self.on_units_text_changed(p_id, text, inp)
        )
        ...
        price_input.textChanged.connect(
            lambda text, p_id=part_id, inp=price_input: self.on_price_text_changed(p_id, text, inp)
        )
        ...
        subtotal_label = QLabel(
            f"{self.format_price(subtotal)} Lei" if subtotal > 0 else "0.00 Lei"
        )
    ...
    def _resize_list(self):
        ...
        if count == 0:
            ...
            return
        total = sum(self.parts_list.sizeHintForRow(i) for i in range(count))
