...
from src.database.models.labor import add_labor as add_labor_to_db
from src.database.models.labor import get_all_labor, get_labor_by_name
...
class LaborSectionWidget(QWidget):
    ...
    def load_data(self, restore_state=False):
        ...
        if restore_state and current_labor:
            ...
            for labor_id in current_labor:
                labor = next((lab for lab in self.all_labor if lab["id"] == labor_id), None)
                if labor:
    ...
    def add_labor(self, labor_id: int, service_name: str):
        ...
        item_widget.setStyleSheet(
            "#laborItem, #laborItem * { background-color: white; } #laborItem { border-bottom: 1px solid #bdc3c7; }"
        )
    ...
    def _resize_list(self):
        ...
        if count == 0:
            ...
            return
        total = sum(self.labor_list.sizeHintForRow(i) for i in range(count))
