...
def _assert_structure(ws, receipt_data: dict):
    ...
    for col in "BCDEF":
        ...
        assert val in (None, ""), (
            f"{col}{manopera_row} trebuie sa fie gol, dar e: {val!r}"
        )
    ...
    assert ws[f"A{piese_row}"].value == "PIESE", (
        ...
        f"dar e: {ws[f'A{piese_row}'].value!r}"
    )
...
def test_returns_path_and_warning_list(tmp_path):
    output_path, warnings = _run.__wrapped__ if hasattr(_run, "__wrapped__") else _run(
        _base_receipt(), tmp_path
    )
...
def test_defects_beyond_5_are_all_written(tmp_path):
    ...
    assert ws["A19"].value == "Bec far ars", (
...
def test_discovered_defects_beyond_5_are_all_written(tmp_path):
    ...
    assert ws["A25"].value == "Bec far ars", (
...
def test_client_parts_beyond_3_are_all_written(tmp_path):
    ...
    assert ws["C15"].value == "Filtru aer"
    assert ws["C16"].value == "Filtru habitaclu"
    assert ws["C17"].value == "Filtru combustibil", (
...
def test_single_labor_written_to_b36(tmp_path):
    ...
    assert ws["B34"].value == "Operatie", "Header 'Operatie' trebuie sa fie la B34 (2 randuri deasupra B36)"
...
def test_labor_total_in_e39_for_3_items(tmp_path):
    ...
    ws, _ = _run(
        _base_receipt(labor=[100, 101, 102], total_labor_cost=900.0), tmp_path
    )
...
def test_multiple_labor_names_written_sequentially(tmp_path):
    ws, _ = _run(
        _base_receipt(labor=[100, 101, 102], total_labor_cost=0.0), tmp_path
    )
    assert ws["B36"].value == "Schimb ulei"
...
def test_billable_part_columns_1_labor_1_part(tmp_path):
    ...
    assert ws["B40"].value == "Piese auto-materiale", "Header 'Piese auto-materiale' trebuie sa fie la B40 cand parts_start_row=42"
...
def test_billable_part_row_no_labor(tmp_path):
    ...
    # parts_start_row=42 → header la B40 (2 randuri mai sus)
    assert ws["B40"].value == "Piese auto-materiale", "Header 'Piese auto-materiale' trebuie sa fie la B40 cand nu exista manopera"
    assert ws["B42"].value == "Filtru ulei",  "Piesa trebuie sa fie la randul 42 cand nu exista manopera"
...
def test_grand_total_row_2_labor_no_parts(tmp_path):
    ...
    ws, _ = _run(
        _base_receipt(labor=[100, 101], total_labor_cost=600.0), tmp_path
    )
...
def test_full_receipt_all_fields(tmp_path):
    ...
    receipt = _base_receipt(
        ...
        # ── Piese facturabile (2) ────────────────────────────────────────
        billable_parts=[
            {"part_id": 10, "part_name": "Filtru ulei",  "units": part1_units, "price_per_unit": part1_price},
            {"part_id": 11, "part_name": "Ulei motor 5W40", "units": part2_units, "price_per_unit": part2_price},
        ],
        total_parts_cost=parts_cost,
    ...
    assert ws["F38"].value == pytest.approx(expected_labor_tva, rel=1e-6), "TVA manoperă incorect în F38"
    ...
    assert ws["B41"].value == "Piese auto-materiale",  "Header 'Piese auto-materiale' incorect/absent in B41"
    ...
    assert ws["E43"].value == pytest.approx(part1_units * part1_price), "Subtotal piesă 1 incorect în E43"
    ...
    assert ws["E44"].value == pytest.approx(part2_units * part2_price), "Subtotal piesă 2 incorect în E44"
    ...
    assert ws["F45"].value == pytest.approx(expected_parts_tva, rel=1e-6), "TVA piese incorect în F45"
