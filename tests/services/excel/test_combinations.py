...
def _make_receipt(
    ...
    if defects is None:
        defects = ALL_DEFECT_IDS
    if discovered_defects is None:
        discovered_defects = ALL_DEFECT_IDS
    if client_parts is None:
        client_parts = ALL_CLIENT_PART_IDS
    if labor is None:
        labor = ALL_LABOR_IDS
    if billable_parts is None:
        billable_parts = ALL_BILLABLE_PARTS
...
def _run_combo(receipt_data: dict, tmp_path):
    ...
    with (
        ...
        patch(
            "src.services.excel_export.get_defect_by_id", side_effect=lambda i: MOCK_DEFECTS.get(i)
        ),
        patch(
            "src.services.excel_export.get_part_by_id",
            side_effect=lambda i: MOCK_CLIENT_PARTS.get(i),
        ),
        patch("src.services.excel_export.get_labor_by_id", side_effect=lambda i: MOCK_LABOR.get(i)),
    ):
...
def _assert_structure(ws, receipt_data: dict):
    ...
    assert ws[f"A{piese_row}"].value == "PIESE", (
        f"A{piese_row} trebuie sa fie 'PIESE', " f"dar e: {ws[f'A{piese_row}'].value!r}  (N={n})"
    )
...
def test_vary_labor(tmp_path, n_labor):
    ...
    if n_labor > 0:
        ...
        assert ws[f"E{total_row}"].value == pytest.approx(
            labor_cost
...
def test_vary_billable_parts(tmp_path, n_bp):
    ...
    for i, part in enumerate(bp):
        ...
        assert ws[f"C{row}"].value == pytest.approx(
            part["units"]
        ), f"Cantitate piesă #{i+1} la C{row} incorectă (n_bp={n_bp})"
        assert ws[f"D{row}"].value == pytest.approx(
            part["price_per_unit"]
        ), f"Preț/unitate piesă #{i+1} la D{row} incorect (n_bp={n_bp})"
        assert ws[f"E{row}"].value == pytest.approx(
            part["units"] * part["price_per_unit"]
...
def test_labor_and_parts_combinations(tmp_path, n_labor, n_bp):
    ...
    for i, part in enumerate(bp):
        row = parts_start_row + i
        assert (
            ws[f"B{row}"].value == part["part_name"]
    ...
    assert ws[f"F{grand_row}"].value == pytest.approx(
