...
class TestLoadForNew:
    def test_resets_state(self, form):
        ...
        assert form.receipt_data == {} or form.receipt_data.get("estimate_cost", None) == 0.0 \
...
_FAKE_RECEIPT = {
    "id": 7, "client_id": 1, "car_id": 11, "kilometers": "1000",
    "executant_name": "Mechanic", "date": "08.05.2026",
    "estimate_cost": 100.0, "estimated_final_date": "10.05.2026",
    "defects": [], "discovered_defects": [], "parts": [],
    "labor": [], "total_labor_cost": 50.0,
    "billable_parts": [], "total_parts_cost": 25.0,
}


class TestLoadForEdit:
    def test_missing_receipt_emits_close(self, form):
        with patch("src.pages.receipts.receipt_form.get_receipt_by_id", return_value=None), \
    ...
    def test_existing_receipt_populates_form(self, form):
        with patch("src.pages.receipts.receipt_form.get_receipt_by_id", return_value=_FAKE_RECEIPT), \
...
class TestOnSaveClicked:
    def test_missing_client_shows_warning(self, form):
        form.receipt_data = {}
        with patch("src.pages.receipts.receipt_form.show_warning") as warn, \
    ...
    def test_new_receipt_calls_add_and_assigns_id(self, form):
        form.receipt_data = {
            "client_id": 1, "plate_number": "P-1", "date": "01.01.2026",
            "total_labor_cost": 50.0, "total_parts_cost": 0.0,
        }
        form.editing_receipt_id = None
        captured = []
        form.receipt_id_assigned.connect(captured.append)
        with patch("src.pages.receipts.receipt_form.get_receipt_by_plate_and_date", return_value=None), \
